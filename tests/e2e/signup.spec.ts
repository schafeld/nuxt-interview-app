...
test.describe('Signup Flow', () => {
  ...
  test('should toggle password visibility', async ({ page }) => {
    ...
    // Check the aria-pressed state instead of type attribute for more reliable testing
    await expect(toggleButton).toHaveAttribute('aria-pressed', 'false')

    // Click show button
    await toggleButton.click()
    await page.waitForTimeout(200) // Brief wait for state change

    // Password should now be visible (aria-pressed should be true)
